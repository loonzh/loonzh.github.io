...
```
##### 1.3Java环境配置
elasticsearch7.4.1自带最合适版本的jdk  
```
# vi /usr/local/elasticsearch-7.4.1/bin/elasticsearch
**配置自己的jdk11**
export JAVA_HOME=/usr/local/elasticsearch-7.4.1/jdk
export PATH=$JAVA_HOME/bin:$PATH

**添加jdk判断**
if [ -x "$JAVA_HOME/bin/java" ]; then
        JAVA="/usr/local/elasticsearch-7.4.1/jdk/bin/java"
else
        JAVA=`which java`
fi
```
#### 2.elasticsearch安装
下载[Zabbix源码](https://www.elastic.co/cn/downloads/past-releases#elasticsearch)并上传到目标机器后执行下面命令：  
```
...
this is test
$ #执行下面的命令测试消费消息
$ bin/kafka-console-consumer.sh --bootstrap-server 192.168.0.xxx:9092 --topic testT
#### 3.logstash安装
```
# tar -xvzf logstash-7.4.1.tar.gz -C /usr/local/elk/
# cd /usr/local/elk/logstash-7.4.1/config
# vim logstash.conf
input {
        kafka {
                codec => "json"
...
#### 4.filebeat安装
```
# tar -xvzf filebeat-7.4.1-linux-x86_64.tar.gz -C /usr/local/elk/
# cd /usr/local/elk/filebeat-7.4.1-linux-x86_64
# vim filebeat.yml
