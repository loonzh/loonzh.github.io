...
tags: [QEMU]
---
**在Windows下直接使QEMU模拟aarch64硬件运行性能很差，所以首先在Windows上使用VMware安装`Kylin V10 x86_64`版本，然后在虚拟机内安装QEMU，最后使用QEMU模拟ARM硬件安装`Kylin V10 aarch64`版本。**  
...
1.下载[ninja](https://github.com/ninja-build/ninja/releases/download/v1.11.1/ninja-linux.zip)并解压  
...
5.下载[qemu-8.0.4.tar.xz](https://download.qemu.org/qemu-8.0.4.tar.xz)并解压  
...
`qemu-img create -f qcow2 kylin-v10-sp3-aarch64.qcow2 120G`  
4.下载引导文件[QEMU_EFI.fd](https://releases.linaro.org/components/kernel/uefi-linaro/latest/release/qemu64/QEMU_EFI.fd)并跟官方镜像和`kylin-v10-sp3-aarch64.qcow2`放在一起  
5.使用官方镜像安装系统(`Kylin-Host-V10-SP3-General-Release-2304-ARM64.iso`和`Kylin-Server-V10-SP3-General-Release-2303-ARM64.iso`不行)  
```
qemu-system-aarch64 -m 1024 -cpu cortex-a72 -smp 2,cores=1,threads=2,sockets=1 -M virt -bios QEMU_EFI.fd -netdev tap,ifname=tap1,id=tapnet,script=no -device virtio-net-device,netdev=tapnet -device nec-usb-xhci -device usb-kbd -device usb-mouse -device VGA -drive if=none,file=Kylin-V10-SP3-2212-ARM64.iso,id=cdrom,media=cdrom -device virtio-scsi-device -device scsi-cd,drive=cdrom -drive if=none,file=kylin-v10-sp3-aarch64.qcow2,id=hd0 -device virtio-blk-device,drive=hd0
```
6.在系统的图形界面下使用`TigerVNC`连接`localhost:5900`安装界面，根据向导完成安装  
7.运行新安装的系统  
```
