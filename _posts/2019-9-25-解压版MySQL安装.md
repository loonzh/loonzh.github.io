...
复制my.cnf文件到etc目录  
[root@localhost ~]# cp /usr/local/mysql/mysql-5.7.22/support-files/my-default.cnf /etc/my.cnf  
修改/etc/my.cnf   
**[client]**    
**port=3306**    
...
mysqld_multi进行多实例管理  
启动全部实例：    [root@localhost ~]#/usr/local/mysql/bin/mysqld_multi start  
查看全部实例状态：[root@localhost ~]#/usr/local/mysql/bin/mysqld_multi report   
启动单个实例：    [root@localhost ~]#/usr/local/mysql/bin/mysqld_multi start 3306   
停止单个实例：    [root@localhost ~]#/usr/local/mysql/bin/mysqld_multi stop 3306   
查看单个实例状态：[root@localhost ~]#/usr/local/mysql/bin/mysqld_multi report 3306   
